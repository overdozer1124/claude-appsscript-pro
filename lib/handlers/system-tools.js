...
function showInfo() {
  ...
  async handleGetScriptInfo(args) {
    try {
      ...
      const fileDetails = files.map(file => ({
        ...
        lastModifyUser: file.lastModifyUser || 'Unknown',
        createTime: file.createTime || 'Unknown',
        updateTime: file.updateTime || 'Unknown',
        sourceLength: (file.source || '').length,
        lineCount: (file.source || '').split('\n').length,
        functionCount: this.countFunctions(file.source || ''),
        hasErrors: this.checkSyntaxIssues(file.source || '')
      }));

      // 重複ファイル検出
      const duplicateFiles = this.detectDuplicateFiles(fileDetails);
      
      // ファイル依存関係分析
      const dependencies = this.analyzeDependencies(fileDetails);

      // Claude出力削減効果計算
      const optimizationAnalysis = this.calculateClaudeOptimization(files);
      
      // 最適化提案生成
      const recommendations = this.generateOptimizationRecommendations(files, duplicateFiles, dependencies);
  ...
  /**
   * Helper: Count functions in file content
   */
  countFunctions(content) {
    if (!content || typeof content !== 'string') return 0;
    
    const functionMatches = content.match(/function\s+\w+\s*\([^)]*\)/g);
    const arrowFunctionMatches = content.match(/const\s+\w+\s*=\s*\([^)]*\)\s*=>/g);
  ...
  /**
   * Helper: Check for syntax issues
   */
  checkSyntaxIssues(content) {
    ...
    if (openParens !== closeParens) {
      issues.push('unmatched_parentheses');
    }
  ...
  /**
   * Helper: Detect duplicate files
   */
  detectDuplicateFiles(fileDetails) {
    const duplicates = [];
    const nameGroups = {};
    
    fileDetails.forEach(file => {
      const baseName = file.name.toLowerCase().replace(/\.(gs|js|html)$/, '');
    ...
    Object.entries(nameGroups).forEach(([baseName, files]) => {
      if (files.length > 1) {
        duplicates.push({
          baseName: baseName,
  ...
  /**
   * Helper: Analyze file dependencies
   */
  analyzeDependencies(fileDetails) {
    const dependencies = [];
    
    fileDetails.forEach(file => {
      const refs = [];
      
      fileDetails.forEach(otherFile => {
        if (file.name === otherFile.name) return;
        
        // Check if this file references other files
        const references = this.findFileReferences(file.source, otherFile.name);
        if (references.length > 0) {
          refs.push({
            referencedFile: otherFile.name,
  ...
  /**
   * Helper: Find references to other files
   */
  findFileReferences(content, fileName) {
    if (!content || !fileName) return [];
    
    const references = [];
    const baseName = fileName.replace(/\.(gs|js)$/, '');
    const searchTerms = [
      fileName,
      baseName,
      `${baseName}(`,
  ...
  generateOptimizationRecommendations(files, duplicateFiles = [], dependencies = []) {
    const recommendations = [];
    const totalFiles = files.length;
    
    // File count based recommendations
    if (totalFiles > 15) {
    ...
    if (duplicateFiles.length > 0) {
      recommendations.push(`🔍 ${duplicateFiles.length} potential duplicate file group(s) detected - consider using delete_script_file`);
    }
    
    // Dependencies recommendations
    if (dependencies.length > 0) {
      const totalRefs = dependencies.reduce((sum, dep) => sum + dep.references.length, 0);
      recommendations.push(`🔗 ${totalRefs} file dependencies detected - use search_files to analyze relationships`);
    }
    
    // General recommendations
    recommendations.push('💡 Use Claude-AppsScript-Pro for output limit problem solving');
    
    if (duplicateFiles.length > 0 || dependencies.length > 0) {
      recommendations.push('🛠️ Use delete_script_file and search_files for project cleanup');
    }
