...
export class FunctionToolsHandler {
   ...
   */
  canHandle(toolName) {
    return ['validate_function_consistency', 'generate_function_stubs', 'analyze_apps_script_function_dependencies'].includes(toolName);
  }
  ...
  async handle(toolName, args) {
    try {
      switch (toolName) {
        ...
        default:
          throw new Error(`Unknown function tool: ${toolName}`);
      }
    } catch (error) {
      this.diagLogger.error(`Function tool error for ${toolName}:`, error);
      throw error;
    }
  }

  /**
   * Route function tool requests to appropriate handlers (deprecated - use handle)
   */
